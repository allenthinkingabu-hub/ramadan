---
name: ipm-market-research-supervisor
description: "Supervisor inspection skill for ipm-market-research. Use when: (1) IPM agent completes self-check for task IPM-INC-002, (2) SupervisorTriggered event received from ipm-market-research skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Market Research — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-INC-002 | Inspection Agent

## Inspection Scope

Independent quality review of the Market Research output for task IPM-INC-002. This supervisor:
- Operates independently from the IPM Market Research agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Market Research agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Market Research agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Market Research Supervisor Inspection Report
