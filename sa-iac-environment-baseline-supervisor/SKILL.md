---
name: sa-iac-environment-baseline-supervisor
description: "Supervisor inspection skill for sa-iac-environment-baseline. Use when: (1) SA agent completes self-check for task IA-DEV-006, (2) SupervisorTriggered event received from sa-iac-environment-baseline skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA IaC & Environment Baseline — Supervisor

Role: System Architect Supervisor | Task ID: IA-DEV-006 | Inspection Agent

## Inspection Scope

Independent quality review of the IaC & Environment Baseline output for task IA-DEV-006. This supervisor:
- Operates independently from the SA IaC & Environment Baseline agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA IaC & Environment Baseline agent sends SupervisorTriggered event with:
...
```
[Trigger] SA IaC & Environment Baseline agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA IaC & Environment Baseline Supervisor Inspection Report
