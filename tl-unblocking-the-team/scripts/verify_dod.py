#!/usr/bin/env python3
"""DoD Self-Verification Script for tl-unblocking-the-team Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <unblocking-the-team-dir>

Example:
    python3 verify_dod.py ./unblocking-the-team
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <unblocking-the-team-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Unblocking report", *check_md_file(base / "unblocking-the-team-report.md")))
    ...
    results.append(("DoD-17", "Blocker resolution report", *check_md_file(base / "blocker-resolution-report.md")))
    results.append(("DoD-18", "Escalation framework", *check_md_file(base / "escalation-framework.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Unblocking the Team (TL-DEV-004)")
