# Trigger Configuration — IPM-DEV-003 PRD Change Management

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IPM-DEV-003 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests prd change management for the project |
...
|----|--------|--------|-------------|
| S-001 | ipm-prd-change-management-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that PRD Change Management is finalized (after supervisor approval) |
