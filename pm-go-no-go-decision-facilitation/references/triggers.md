# Trigger Configuration — PM-QA-002 Go/No-Go Decision Facilitation

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches PM-QA-002 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests go/no-go decision facilitation for the project |
...
|----|--------|--------|-------------|
| S-001 | pm-go-no-go-decision-facilitation-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Go/No-Go Decision Facilitation is finalized (after supervisor approval) |
