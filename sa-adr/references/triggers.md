# Trigger Configuration — IA-DEV-005 Architecture Decision Records (ADRs)

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-DEV-005 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests architecture decision records (adrs) for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-adr-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Architecture Decision Records (ADRs) is finalized (after supervisor approval) |
