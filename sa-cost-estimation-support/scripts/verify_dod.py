#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-cost-estimation-support Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <cost-estimation-dir>

Example:
    python3 verify_dod.py ./cost-estimation
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <cost-estimation-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Cost Estimation report", *check_md_file(base / "cost-estimation-report.md")))
    ...
    results.append(("DoD-17", "Cost breakdown structure", *check_md_file(base / "cost-breakdown.md")))
    results.append(("DoD-18", "TCO projection", *check_md_file(base / "tco-projection.md")))
    results.append(("DoD-19", "FinOps guardrails", *check_md_file(base / "finops-guardrails.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Cost Estimation Support (IA-INC-005)")
