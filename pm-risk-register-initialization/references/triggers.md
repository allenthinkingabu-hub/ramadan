# Trigger Configuration — PM-INC-005 Risk Register Initialization

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches PM-INC-005 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests risk register initialization for the project |
...
|----|--------|--------|-------------|
| S-001 | pm-risk-register-initialization-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Risk Register Initialization is finalized (after supervisor approval) |
