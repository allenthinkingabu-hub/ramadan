# Trigger Configuration — IPM-QA-001 UAT Organization

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IPM-QA-001 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests uat organization for the project |
...
|----|--------|--------|-------------|
| S-001 | ipm-uat-organization-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that UAT Organization is finalized (after supervisor approval) |
