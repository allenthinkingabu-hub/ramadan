---
name: ipm-requirement-clarification-supervisor
description: "Supervisor inspection skill for ipm-requirement-clarification. Use when: (1) IPM agent completes self-check for task IPM-DEV-001, (2) SupervisorTriggered event received from ipm-requirement-clarification skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Requirement Clarification — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-DEV-001 | Inspection Agent

## Inspection Scope

Independent quality review of the Requirement Clarification output for task IPM-DEV-001. This supervisor:
- Operates independently from the IPM Requirement Clarification agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Requirement Clarification agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Requirement Clarification agent sends SupervisorTriggered
     |
     v
...
[Decide] Pass rate = 100%?
     |-- No  -> Return report to IPM agent for remediation
...
```markdown
# IPM Requirement Clarification Supervisor Inspection Report
...
## Conclusion: [FAIL -> Return to IPM Agent | PASS -> Notify PM Agent]
```
