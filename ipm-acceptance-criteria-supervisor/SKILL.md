---
name: ipm-acceptance-criteria-supervisor
description: "Supervisor inspection skill for ipm-acceptance-criteria. Use when: (1) IPM agent completes self-check for task IPM-DEV-002, (2) SupervisorTriggered event received from ipm-acceptance-criteria skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Acceptance Criteria — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-DEV-002 | Inspection Agent

## Inspection Scope

Independent quality review of the Acceptance Criteria output for task IPM-DEV-002. This supervisor:
- Operates independently from the IPM Acceptance Criteria agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Acceptance Criteria agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Acceptance Criteria agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Acceptance Criteria Supervisor Inspection Report
