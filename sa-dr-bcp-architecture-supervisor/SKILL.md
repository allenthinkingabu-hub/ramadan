---
name: sa-dr-bcp-architecture-supervisor
description: "Supervisor inspection skill for sa-dr-bcp-architecture. Use when: (1) SA agent completes self-check for task IA-REQ-007, (2) SupervisorTriggered event received from sa-dr-bcp-architecture skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA DR/BCP Architecture — Supervisor

Role: System Architect Supervisor | Task ID: IA-REQ-007 | Inspection Agent

## Inspection Scope

Independent quality review of the DR/BCP Architecture output for task IA-REQ-007. This supervisor:
- Operates independently from the SA DR/BCP Architecture agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA DR/BCP Architecture agent sends SupervisorTriggered event with:
...
```
[Trigger] SA DR/BCP Architecture agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA DR/BCP Architecture Supervisor Inspection Report
