---
name: pm-scope-management-supervisor
description: "Supervisor inspection skill for pm-scope-management. Use when: (1) PM agent completes self-check for task PM-DEV-003, (2) SupervisorTriggered event received from pm-scope-management skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# PM Scope Management — Supervisor

Role: IT Project Manager Supervisor | Task ID: PM-DEV-003 | Inspection Agent

## Inspection Scope

Independent quality review of the Scope Management output for task PM-DEV-003. This supervisor:
- Operates independently from the PM Scope Management agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the PM Scope Management agent sends SupervisorTriggered event with:
...
```
[Trigger] PM Scope Management agent sends SupervisorTriggered
     |
     v
...
```markdown
# PM Scope Management Supervisor Inspection Report
