#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-risk-identification Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <risk-identification-dir>

Example:
    python3 verify_dod.py ./risk-identification
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <risk-identification-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Risk Identification report", *check_md_file(base / "risk-identification-report.md")))
    ...
    results.append(("DoD-17", "Risk register", *check_md_file(base / "risk-register.md")))
    results.append(("DoD-18", "Dependency map", *check_md_file(base / "diagrams" / "dependency-map.md")))
    results.append(("DoD-19", "Mitigation strategies", *check_md_file(base / "mitigation-strategies.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Risk Identification (IA-INC-004)")
