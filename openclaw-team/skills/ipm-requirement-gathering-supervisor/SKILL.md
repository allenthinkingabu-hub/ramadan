---
name: ipm-requirement-gathering-supervisor
description: "Supervisor inspection skill for ipm-requirement-gathering. Use when: (1) IPM agent completes self-check for task IPM-INC-001, (2) SupervisorTriggered event received from ipm-requirement-gathering skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Requirement Gathering — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-INC-001 | Inspection Agent

## Inspection Scope

Independent quality review of the Requirement Gathering output for task IPM-INC-001. This supervisor:
- Operates independently from the IPM Requirement Gathering agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Requirement Gathering agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Requirement Gathering agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Requirement Gathering Supervisor Inspection Report
