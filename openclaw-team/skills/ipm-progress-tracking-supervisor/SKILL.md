---
name: ipm-progress-tracking-supervisor
description: "Supervisor inspection skill for ipm-progress-tracking. Use when: (1) IPM agent completes self-check for task IPM-DEV-004, (2) SupervisorTriggered event received from ipm-progress-tracking skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Progress Tracking — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-DEV-004 | Inspection Agent

## Inspection Scope

Independent quality review of the Progress Tracking output for task IPM-DEV-004. This supervisor:
- Operates independently from the IPM Progress Tracking agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Progress Tracking agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Progress Tracking agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Progress Tracking Supervisor Inspection Report
