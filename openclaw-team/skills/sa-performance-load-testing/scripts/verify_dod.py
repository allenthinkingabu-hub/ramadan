#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-performance-load-testing Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <performance-load-testing-dir>

Example:
    python3 verify_dod.py ./performance-load-testing
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <performance-load-testing-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Performance Load Testing report", *check_md_file(base / "performance-load-testing-report.md")))
    ...
    results.append(("DoD-17", "Test scenarios", *check_directory(base / "scenarios", min_files=1)))
    results.append(("DoD-18", "Benchmark definitions", *check_md_file(base / "scenarios" / "benchmarks.md")))
    results.append(("DoD-19", "Test execution plan", *check_md_file(base / "test-execution-plan.md")))
    results.append(("DoD-20", "Acceptance criteria matrix", *check_md_file(base / "acceptance-criteria.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Performance & Load Testing (IA-QA-001)")
