#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-code-design-reviews Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <code-design-reviews-dir>

Example:
    python3 verify_dod.py ./code-design-reviews
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <code-design-reviews-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Code & Design Review report", *check_md_file(base / "code-design-review-report.md")))
    ...
    results.append(("DoD-17", "Compliance matrix", *check_md_file(base / "compliance-matrix.md")))
    results.append(("DoD-18", "Remediation plan", *check_md_file(base / "remediation-plan.md")))
    results.append(("DoD-19", "Review decision log", *check_md_file(base / "review-decision-log.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Code & Design Reviews (IA-DEV-002)")
