# Trigger Configuration — IA-QA-003 Infrastructure Validation

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-QA-003 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests infrastructure validation for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-infrastructure-validation-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Infrastructure Validation is finalized (after supervisor approval) |
