#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-data-privacy-governance Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <data-privacy-governance-dir>

Example:
    python3 verify_dod.py ./data-privacy-governance
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <data-privacy-governance-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Data Privacy & Governance report", *check_md_file(base / "data-privacy-governance-report.md")))
    ...
    results.append(("DoD-17", "Governance diagrams", *check_directory(base / "diagrams", min_files=2)))
    results.append(("DoD-18", "Regulatory compliance view", *check_md_file(base / "diagrams" / "regulatory-compliance-view.md")))
    results.append(("DoD-19", "NFR alignment note", *check_md_file(base / "nfr-alignment.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Data Privacy & Governance (IA-REQ-006)")
