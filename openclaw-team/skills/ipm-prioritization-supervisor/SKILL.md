---
name: ipm-prioritization-supervisor
description: "Supervisor inspection skill for ipm-prioritization. Use when: (1) IPM agent completes self-check for task IPM-REQ-003, (2) SupervisorTriggered event received from ipm-prioritization skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Prioritization — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-REQ-003 | Inspection Agent

## Inspection Scope

Independent quality review of the Prioritization output for task IPM-REQ-003. This supervisor:
- Operates independently from the IPM Prioritization agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Prioritization agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Prioritization agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Prioritization Supervisor Inspection Report
