#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-spike-poc-leadership Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <spike-poc-dir>

Example:
    python3 verify_dod.py ./spike-poc
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <spike-poc-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Spike/PoC report", *check_md_file(base / "spike-poc-report.md")))
    ...
    results.append(("DoD-17", "Technology comparison", *check_md_file(findings / "technology-comparison.md")))
    results.append(("DoD-18", "Recommendation & decision record", *check_md_file(findings / "recommendation-decision.md")))
    results.append(("DoD-19", "Risk assessment update", *check_md_file(findings / "risk-assessment.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Spike & PoC Leadership (IA-DEV-003)")
