#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-architecture-design Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <architecture-design-dir>

Example:
    python3 verify_dod.py ./architecture-design
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <architecture-design-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Architecture Design report", *check_md_file(base / "architecture-design-report.md")))
    ...
    results.append(("DoD-17", "Architecture diagrams", *check_directory(base / "diagrams", min_files=2)))
    results.append(("DoD-18", "Interface/Integration view", *check_md_file(base / "diagrams" / "integration-view.md")))
    ...
    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Architecture Design (IA-REQ-001)")
