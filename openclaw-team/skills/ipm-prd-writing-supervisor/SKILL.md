---
name: ipm-prd-writing-supervisor
description: "Supervisor inspection skill for ipm-prd-writing. Use when: (1) IPM agent completes self-check for task IPM-REQ-001, (2) SupervisorTriggered event received from ipm-prd-writing skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM PRD Writing — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-REQ-001 | Inspection Agent

## Inspection Scope

Independent quality review of the PRD Writing output for task IPM-REQ-001. This supervisor:
- Operates independently from the IPM PRD Writing agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM PRD Writing agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM PRD Writing agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM PRD Writing Supervisor Inspection Report
