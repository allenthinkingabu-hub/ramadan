# Trigger Configuration — PM-INC-004 Budget Estimation

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches PM-INC-004 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests budget estimation for the project |
...
|----|--------|--------|-------------|
| S-001 | pm-budget-estimation-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Budget Estimation is finalized (after supervisor approval) |
