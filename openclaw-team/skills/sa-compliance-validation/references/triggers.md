# Trigger Configuration — IA-QA-005 Compliance Validation

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-QA-005 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests compliance validation for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-compliance-validation-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Compliance Validation is finalized (after supervisor approval) |
