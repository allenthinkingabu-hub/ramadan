---
name: pm-resource-planning-supervisor
description: "Supervisor inspection skill for pm-resource-planning. Use when: (1) PM agent completes self-check for task PM-INC-003, (2) SupervisorTriggered event received from pm-resource-planning skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# PM Resource Planning — Supervisor

Role: IT Project Manager Supervisor | Task ID: PM-INC-003 | Inspection Agent

## Inspection Scope

Independent quality review of the Resource Planning output for task PM-INC-003. This supervisor:
- Operates independently from the PM Resource Planning agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the PM Resource Planning agent sends SupervisorTriggered event with:
...
```
[Trigger] PM Resource Planning agent sends SupervisorTriggered
     |
     v
...
```markdown
# PM Resource Planning Supervisor Inspection Report
