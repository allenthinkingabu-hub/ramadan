#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-technical-standards Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <technical-standards-dir>

Example:
    python3 verify_dod.py ./technical-standards
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <technical-standards-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Technical Standards report", *check_md_file(base / "technical-standards-report.md")))
    ...
    results.append(("DoD-17", "Standards documentation", *check_directory(base / "diagrams", min_files=1)))
    results.append(("DoD-18", "Standards enforcement view", *check_md_file(base / "diagrams" / "standards-enforcement-view.md")))
    ...
    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Technical Standards (IA-REQ-005)")
