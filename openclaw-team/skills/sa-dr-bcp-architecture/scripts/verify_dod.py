#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-dr-bcp-architecture Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <dr-bcp-architecture-dir>

Example:
    python3 verify_dod.py ./dr-bcp-architecture
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <dr-bcp-architecture-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "DR/BCP Architecture report", *check_md_file(base / "dr-bcp-architecture-report.md")))
    ...
    results.append(("DoD-17", "DR/BCP diagrams", *check_directory(base / "diagrams", min_files=2)))
    results.append(("DoD-18", "Resilience architecture view", *check_md_file(base / "diagrams" / "resilience-architecture-view.md")))
    ...
    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- DR/BCP Architecture (IA-REQ-007)")
