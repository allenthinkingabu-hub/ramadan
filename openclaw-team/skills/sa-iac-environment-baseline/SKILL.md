---
name: sa-iac-environment-baseline
description: "System Architect skill for Establishing reproducible environments (IaC, configuration management) and enforcing drift controls across dev/test/stage/prod. Use when: (1) PM assigns task IA-DEV-006 via TaskTriggered event, (2) Development phase iac & environment baseline activities are needed, (3) orchestrating iac & environment baseline deliverables for downstream team tasks."
---

# SA IaC & Environment Baseline

Role: System Architect | Task ID: IA-DEV-006 | Wave: 10, Step: 6

## Objective

Establishing reproducible environments (IaC, configuration management) and enforcing drift controls across dev/test/stage/prod.

## Upstream Inputs

- **Solutions Architect outputs**: SAD, Integration Architecture, NFR Mapping, Technology Blueprint
- **Previous SA phase outputs**: Completed Development prerequisites

## Downstream Triggers

On TaskCompleted (after supervisor 100% pass):
- TL-DEV-001, TL-DEV-002, TL-DEV-003, TL-DEV-004, TL-DEV-005

## Workflow (Phase 0-5)

### Phase 0: Initialization

1. Create output directory: `~/.openclaw/workspace-sa/outputs/IA-DEV-006/`
...
2. Analyze and summarize understanding of the **task purpose** (why iac & environment baseline is needed)
...
7. Invoke supervisor: send SupervisorTriggered event to `sa-iac-environment-baseline-supervisor`
