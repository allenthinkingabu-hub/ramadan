---
name: ipm-bug-prioritization-supervisor
description: "Supervisor inspection skill for ipm-bug-prioritization. Use when: (1) IPM agent completes self-check for task IPM-QA-002, (2) SupervisorTriggered event received from ipm-bug-prioritization skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Bug Prioritization — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-QA-002 | Inspection Agent

## Inspection Scope

Independent quality review of the Bug Prioritization output for task IPM-QA-002. This supervisor:
- Operates independently from the IPM Bug Prioritization agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Bug Prioritization agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Bug Prioritization agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Bug Prioritization Supervisor Inspection Report
