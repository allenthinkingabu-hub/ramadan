# Trigger Configuration — PM-DEV-003 Scope Management

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches PM-DEV-003 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests scope management for the project |
...
|----|--------|--------|-------------|
| S-001 | pm-scope-management-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Scope Management is finalized (after supervisor approval) |
