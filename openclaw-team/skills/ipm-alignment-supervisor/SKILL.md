---
name: ipm-alignment-supervisor
description: "Supervisor inspection skill for ipm-alignment. Use when: (1) IPM agent completes self-check for task IPM-INC-005, (2) SupervisorTriggered event received from ipm-alignment skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Alignment — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-INC-005 | Inspection Agent

## Inspection Scope

Independent quality review of the Alignment output for task IPM-INC-005. This supervisor:
- Operates independently from the IPM Alignment agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Alignment agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Alignment agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Alignment Supervisor Inspection Report
