---
name: sa-finops-cost-guardrails-supervisor
description: "Supervisor inspection skill for sa-finops-cost-guardrails. Use when: (1) SA agent completes self-check for task IA-REL-007, (2) SupervisorTriggered event received from sa-finops-cost-guardrails skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA FinOps & Cost Guardrails — Supervisor

Role: System Architect Supervisor | Task ID: IA-REL-007 | Inspection Agent

## Inspection Scope

Independent quality review of the FinOps & Cost Guardrails output for task IA-REL-007. This supervisor:
- Operates independently from the SA FinOps & Cost Guardrails agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA FinOps & Cost Guardrails agent sends SupervisorTriggered event with:
...
```
[Trigger] SA FinOps & Cost Guardrails agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA FinOps & Cost Guardrails Supervisor Inspection Report
