# Trigger Configuration — IA-REL-004 Post-Mortem Analysis

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-REL-004 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests post-mortem analysis for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-post-mortem-analysis-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Post-Mortem Analysis is finalized (after supervisor approval) |
