---
name: ipm-user-feedback-supervisor
description: "Supervisor inspection skill for ipm-user-feedback. Use when: (1) IPM agent completes self-check for task IPM-REL-003, (2) SupervisorTriggered event received from ipm-user-feedback skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM User Feedback — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-REL-003 | Inspection Agent

## Inspection Scope

Independent quality review of the User Feedback output for task IPM-REL-003. This supervisor:
- Operates independently from the IPM User Feedback agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM User Feedback agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM User Feedback agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM User Feedback Supervisor Inspection Report
