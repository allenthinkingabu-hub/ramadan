---
name: pm-communication-plan-supervisor
description: "Supervisor inspection skill for pm-communication-plan. Use when: (1) PM agent completes self-check for task PM-REQ-004, (2) SupervisorTriggered event received from pm-communication-plan skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# PM Communication Plan — Supervisor

Role: IT Project Manager Supervisor | Task ID: PM-REQ-004 | Inspection Agent

## Inspection Scope

Independent quality review of the Communication Plan output for task PM-REQ-004. This supervisor:
- Operates independently from the PM Communication Plan agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the PM Communication Plan agent sends SupervisorTriggered event with:
...
```
[Trigger] PM Communication Plan agent sends SupervisorTriggered
     |
     v
...
```markdown
# PM Communication Plan Supervisor Inspection Report
