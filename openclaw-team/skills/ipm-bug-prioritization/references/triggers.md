# Trigger Configuration — IPM-QA-002 Bug Prioritization

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IPM-QA-002 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests bug prioritization for the project |
...
|----|--------|--------|-------------|
| S-001 | ipm-bug-prioritization-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Bug Prioritization is finalized (after supervisor approval) |
