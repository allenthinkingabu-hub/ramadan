---
name: sa-adr-supervisor
description: "Supervisor inspection skill for sa-adr. Use when: (1) SA agent completes self-check for task IA-DEV-005, (2) SupervisorTriggered event received from sa-adr skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA Architecture Decision Records (ADRs) — Supervisor

Role: System Architect Supervisor | Task ID: IA-DEV-005 | Inspection Agent

## Inspection Scope

Independent quality review of the Architecture Decision Records (ADRs) output for task IA-DEV-005. This supervisor:
- Operates independently from the SA Architecture Decision Records (ADRs) agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA Architecture Decision Records (ADRs) agent sends SupervisorTriggered event with:
...
```
[Trigger] SA Architecture Decision Records (ADRs) agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA Architecture Decision Records (ADRs) Supervisor Inspection Report
