---
name: pm-go-no-go-decision-facilitation-supervisor
description: "Supervisor inspection skill for pm-go-no-go-decision-facilitation. Use when: (1) PM agent completes self-check for task PM-QA-002, (2) SupervisorTriggered event received from pm-go-no-go-decision-facilitation skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# PM Go/No-Go Decision Facilitation — Supervisor

Role: IT Project Manager Supervisor | Task ID: PM-QA-002 | Inspection Agent

## Inspection Scope

Independent quality review of the Go/No-Go Decision Facilitation output for task PM-QA-002. This supervisor:
- Operates independently from the PM Go/No-Go Decision Facilitation agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the PM Go/No-Go Decision Facilitation agent sends SupervisorTriggered event with:
...
```
[Trigger] PM Go/No-Go Decision Facilitation agent sends SupervisorTriggered
     |
     v
...
[Decide] Pass rate = 100%?
     |-- No  -> Return report to PM agent for remediation
...
```markdown
# PM Go/No-Go Decision Facilitation Supervisor Inspection Report
...
## Conclusion: [FAIL -> Return to PM Agent | PASS -> Notify PM Agent]
```
