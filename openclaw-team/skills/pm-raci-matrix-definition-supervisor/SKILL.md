---
name: pm-raci-matrix-definition-supervisor
description: "Supervisor inspection skill for pm-raci-matrix-definition. Use when: (1) PM agent completes self-check for task PM-REQ-003, (2) SupervisorTriggered event received from pm-raci-matrix-definition skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# PM RACI Matrix Definition — Supervisor

Role: IT Project Manager Supervisor | Task ID: PM-REQ-003 | Inspection Agent

## Inspection Scope

Independent quality review of the RACI Matrix Definition output for task PM-REQ-003. This supervisor:
- Operates independently from the PM RACI Matrix Definition agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the PM RACI Matrix Definition agent sends SupervisorTriggered event with:
...
```
[Trigger] PM RACI Matrix Definition agent sends SupervisorTriggered
     |
     v
...
```markdown
# PM RACI Matrix Definition Supervisor Inspection Report
