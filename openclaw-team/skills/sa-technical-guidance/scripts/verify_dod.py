#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-technical-guidance Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <technical-guidance-dir>

Example:
    python3 verify_dod.py ./technical-guidance
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <technical-guidance-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Technical Guidance report", *check_md_file(base / "technical-guidance-report.md")))
    ...
    results.append(("DoD-17", "Pattern catalog", *check_md_file(base / "patterns" / "pattern-catalog.md")))
    results.append(("DoD-18", "Architecture-to-code mapping", *check_md_file(base / "architecture-code-mapping.md")))
    results.append(("DoD-19", "Guidance decision log", *check_md_file(base / "guidance-decision-log.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Technical Guidance (IA-DEV-001)")
