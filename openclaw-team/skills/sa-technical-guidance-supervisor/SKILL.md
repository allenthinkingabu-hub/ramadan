---
name: sa-technical-guidance-supervisor
description: "Independent Quality Supervisor AI Agent skill for end-to-end quality monitoring and closed-loop remediation of the sa-technical-guidance agent output. Use when: (1) the Technical Guidance Agent (IA-DEV-001) completes one round of output and needs quality inspection, (2) verifying all required deliverables of the technical guidance task are complete and valid (including pattern catalog and architecture-to-code mapping), or (3) ensuring closed-loop quality remediation before handoff to PM Agent."
---

# SA Technical Guidance Supervisor Agent

Role: Quality Supervisor | Parent Task: IA-DEV-001 Technical Guidance | Wave: 10, Step: 1

Operates independently from the Technical Guidance Agent. Does not participate in the guidance -- only reviews and provides feedback.

## Trigger

Automatically triggered after the Technical Guidance AI Agent completes one round of output.
...
| 14 | Pattern catalog | `patterns/pattern-catalog.md` exists with design patterns, selection criteria, trade-offs |
| 15 | Architecture-to-code mapping | `architecture-code-mapping.md` exists with component-to-module mapping and dependency rules |
| 16 | Guidance decision log | `guidance-decision-log.md` exists with decisions, rationale, alternatives considered |
| 17 | Upstream traceability | IA-REQ-001, SA-REQ references present, guidance traceable to architecture decisions |

## Inspection Process

```
[Trigger] Technical Guidance Agent completes output
     |
...
[Decide] Pass rate = 100%?
     +-- No -> Return report to Technical Guidance Agent for item-by-item remediation
...
```markdown
# Technical Guidance Supervisor Inspection Report

- Inspection Time: {timestamp}
- Inspection Round: #{N}
- Technical Guidance Report File Path: {file_path}
...
| Item 14: Pattern Catalog | PASS / FAIL | {notes} |
| Item 15: Architecture-to-Code Mapping | PASS / FAIL | {notes} |
| Item 16: Guidance Decision Log | PASS / FAIL | {notes} |
...
2. Invoke PM Agent, sending:
   - Technical Guidance report file path and filename
