---
name: sa-compliance-validation-supervisor
description: "Supervisor inspection skill for sa-compliance-validation. Use when: (1) SA agent completes self-check for task IA-QA-005, (2) SupervisorTriggered event received from sa-compliance-validation skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA Compliance Validation — Supervisor

Role: System Architect Supervisor | Task ID: IA-QA-005 | Inspection Agent

## Inspection Scope

Independent quality review of the Compliance Validation output for task IA-QA-005. This supervisor:
- Operates independently from the SA Compliance Validation agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA Compliance Validation agent sends SupervisorTriggered event with:
...
```
[Trigger] SA Compliance Validation agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA Compliance Validation Supervisor Inspection Report
