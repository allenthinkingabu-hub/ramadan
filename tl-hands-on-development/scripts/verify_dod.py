#!/usr/bin/env python3
"""DoD Self-Verification Script for tl-hands-on-development Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <hands-on-development-dir>

Example:
    python3 verify_dod.py ./hands-on-development
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <hands-on-development-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Hands-On Development report",
                     *check_md_file(base / "hands-on-development-report.md")))
    ...
    results.append(("DoD-17", "Implementation patterns", *check_md_file(base / "implementation-patterns.md")))
    results.append(("DoD-18", "Reference implementations", *check_directory(base / "implementations")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Hands-On Development (TL-DEV-002)")
