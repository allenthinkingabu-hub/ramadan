---
name: sa-third-party-vendor-strategy-supervisor
description: "Supervisor inspection skill for sa-third-party-vendor-strategy. Use when: (1) SA agent completes self-check for task IA-INC-008, (2) SupervisorTriggered event received from sa-third-party-vendor-strategy skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA Third-Party/Vendor Strategy — Supervisor

Role: System Architect Supervisor | Task ID: IA-INC-008 | Inspection Agent

## Inspection Scope

Independent quality review of the Third-Party/Vendor Strategy output for task IA-INC-008. This supervisor:
- Operates independently from the SA Third-Party/Vendor Strategy agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA Third-Party/Vendor Strategy agent sends SupervisorTriggered event with:
...
```
[Trigger] SA Third-Party/Vendor Strategy agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA Third-Party/Vendor Strategy Supervisor Inspection Report
