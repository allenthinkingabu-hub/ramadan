#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-security-review Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <security-review-dir>

Example:
    python3 verify_dod.py ./security-review
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <security-review-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Security Review report", *check_md_file(base / "security-review-report.md")))
    ...
    results.append(("DoD-17", "Threat Model", *check_md_file(base / "assessments" / "threat-model.md")))
    results.append(("DoD-18", "Vulnerability Assessment", *check_md_file(base / "assessments" / "vulnerability-assessment.md")))
    results.append(("DoD-19", "Security Policy Compliance", *check_md_file(base / "security-policy-compliance.md")))
    results.append(("DoD-20", "Security Test Plan", *check_md_file(base / "security-test-plan.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Security Review (IA-QA-002)")
