#!/usr/bin/env python3
"""DoD Self-Verification Script for tl-technical-vision-direction Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <technical-vision-direction-dir>

Example:
    python3 verify_dod.py ./technical-vision-direction
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <technical-vision-direction-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Technical Vision Direction report", *check_md_file(base / "technical-vision-direction-report.md")))
    ...
    results.append(("DoD-17", "Technical Vision Statement", *check_md_file(base / "technical-vision-statement.md")))
    results.append(("DoD-18", "Guiding Principles", *check_md_file(base / "guiding-principles.md")))
    results.append(("DoD-19", "Business-Technology Alignment Matrix", *check_md_file(base / "alignment-matrix.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Technical Vision & Direction (TL-INC-001)")
