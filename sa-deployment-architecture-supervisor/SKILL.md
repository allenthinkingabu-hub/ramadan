---
name: sa-deployment-architecture-supervisor
description: "Supervisor inspection skill for sa-deployment-architecture. Use when: (1) SA agent completes self-check for task IA-REL-001, (2) SupervisorTriggered event received from sa-deployment-architecture skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA Deployment Architecture — Supervisor

Role: System Architect Supervisor | Task ID: IA-REL-001 | Inspection Agent

## Inspection Scope

Independent quality review of the Deployment Architecture output for task IA-REL-001. This supervisor:
- Operates independently from the SA Deployment Architecture agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA Deployment Architecture agent sends SupervisorTriggered event with:
...
```
[Trigger] SA Deployment Architecture agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA Deployment Architecture Supervisor Inspection Report
