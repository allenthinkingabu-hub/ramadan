---
name: tl-technical-decision-making-supervisor
description: "Independent Quality Supervisor AI Agent skill for end-to-end quality monitoring and closed-loop remediation of the tl-technical-decision-making agent output. Use when: (1) the Technical Decision Making Agent (TL-DEV-003) completes one round of output and needs quality inspection, (2) verifying all required deliverables are complete and valid, or (3) ensuring closed-loop quality remediation before handoff to PM Agent."
---

# TL Technical Decision Making Supervisor Agent

Role: Quality Supervisor | Parent Task: TL-DEV-003 Technical Decision Making | Wave: 11, Step: 3

Operates independently from the Technical Decision Making Agent. Does not participate in the work -- only reviews and provides feedback.

## Trigger

Automatically triggered after the Technical Decision Making AI Agent completes one round of output.
...
| 14 | Trade-off analysis | `trade-off-analysis.md` exists with options, scoring criteria, recommendations |
| 15 | Decision register | `decision-register.md` exists with all decisions, rationale, status |
| 16 | ADR quality | Decision records include context, alternatives, consequences, rationale |
| 17 | Upstream traceability | Architecture design references present, decisions aligned with architecture |

## Inspection Process

```
[Trigger] Technical Decision Making Agent completes output
     |
...
```markdown
# Technical Decision Making Supervisor Inspection Report
...
| Item 14: Trade-off Analysis | PASS / FAIL | {notes} |
| Item 15: Decision Register | PASS / FAIL | {notes} |
| Item 16: ADR Quality | PASS / FAIL | {notes} |
...
2. Invoke PM Agent, sending:
   - Technical Decision Making report file path and filename
