# Trigger Configuration — IPM-INC-005 Alignment

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IPM-INC-005 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests alignment for the project |
...
|----|--------|--------|-------------|
| S-001 | ipm-alignment-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Alignment is finalized (after supervisor approval) |
