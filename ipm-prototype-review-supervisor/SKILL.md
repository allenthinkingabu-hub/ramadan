---
name: ipm-prototype-review-supervisor
description: "Supervisor inspection skill for ipm-prototype-review. Use when: (1) IPM agent completes self-check for task IPM-REQ-004, (2) SupervisorTriggered event received from ipm-prototype-review skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM Prototype Review — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-REQ-004 | Inspection Agent

## Inspection Scope

Independent quality review of the Prototype Review output for task IPM-REQ-004. This supervisor:
- Operates independently from the IPM Prototype Review agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM Prototype Review agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM Prototype Review agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM Prototype Review Supervisor Inspection Report
