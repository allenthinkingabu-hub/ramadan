# Trigger Configuration — IA-REQ-008 Vendor & Third-Party Controls

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-REQ-008 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests vendor & third-party controls for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-vendor-third-party-controls-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Vendor & Third-Party Controls is finalized (after supervisor approval) |
