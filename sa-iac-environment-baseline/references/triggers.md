# Trigger Configuration — IA-DEV-006 IaC & Environment Baseline

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-DEV-006 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests iac & environment baseline for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-iac-environment-baseline-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that IaC & Environment Baseline is finalized (after supervisor approval) |
