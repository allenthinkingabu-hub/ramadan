#!/usr/bin/env python3
"""DoD Self-Verification Script for tl-technology-stack-decision Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <technology-stack-decision-dir>

Example:
    python3 verify_dod.py ./technology-stack-decision
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <technology-stack-decision-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Technology Stack Decision report", *check_md_file(base / "technology-stack-decision-report.md")))
    ...
    results.append(("DoD-17", "Technology Decision Matrix", *check_md_file(base / "technology-decision-matrix.md")))
    results.append(("DoD-18", "Stack Specification", *check_md_file(base / "stack-specification.md")))
    results.append(("DoD-19", "Compatibility Analysis", *check_md_file(base / "compatibility-analysis.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Technology Stack Decision (TL-INC-002)")
