# Trigger Configuration — IA-REL-001 Deployment Architecture

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IA-REL-001 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests deployment architecture for the project |
...
|----|--------|--------|-------------|
| S-001 | sa-deployment-architecture-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Deployment Architecture is finalized (after supervisor approval) |
