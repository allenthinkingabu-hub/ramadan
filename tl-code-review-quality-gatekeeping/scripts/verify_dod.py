#!/usr/bin/env python3
"""DoD Self-Verification Script for tl-code-review-quality-gatekeeping Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <code-review-quality-gatekeeping-dir>

Example:
    python3 verify_dod.py ./code-review-quality-gatekeeping
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <code-review-quality-gatekeeping-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Code Review Quality Gatekeeping report",
                     *check_md_file(base / "code-review-quality-gatekeeping-report.md")))
    ...
    results.append(("DoD-17", "Review standards document", *check_md_file(base / "review-standards.md")))
    results.append(("DoD-18", "Quality gate report", *check_md_file(base / "quality-gate-report.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Code Review & Quality Gatekeeping (TL-DEV-001)")
