#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-stakeholder-alignment Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <stakeholder-alignment-dir>

Example:
    python3 verify_dod.py ./stakeholder-alignment
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <stakeholder-alignment-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "Stakeholder Alignment report", *check_md_file(base / "stakeholder-alignment-report.md")))
    ...
    results.append(("DoD-17", "Stakeholder map", *check_md_file(base / "stakeholder-map.md")))
    results.append(("DoD-18", "Decision criteria matrix", *check_md_file(base / "decision-criteria.md")))
    results.append(("DoD-19", "Workshop outcomes", *check_md_file(base / "workshop-outcomes.md")))

    passed = sum(1 for r in results if r[2])
    total = len(results)

    print(f"\n{'='*70}")
    print(f"DoD Verification Report -- Stakeholder Alignment (IA-INC-006)")
