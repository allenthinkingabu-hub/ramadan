#!/usr/bin/env python3
"""DoD Self-Verification Script for sa-non-functional-requirements Agent.

Checks all DoD items and reports pass/fail status.

Usage:
    python3 verify_dod.py <non-functional-requirements-dir>

Example:
    python3 verify_dod.py ./non-functional-requirements
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 verify_dod.py <non-functional-requirements-dir>")
        sys.exit(1)
    ...
    results.append(("DoD-14", "NFR specification report", *check_md_file(base / "nfr-report.md")))
    ...
    print(f"\n{'='*70}")
    print(f"DoD Verification Report — Non-Functional Requirements (IA-REQ-002)")
