# Trigger Configuration — IPM-DEV-004 Progress Tracking

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IPM-DEV-004 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests progress tracking for the project |
...
|----|--------|--------|-------------|
| S-001 | ipm-progress-tracking-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Progress Tracking is finalized (after supervisor approval) |
