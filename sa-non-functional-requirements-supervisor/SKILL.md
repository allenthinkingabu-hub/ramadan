---
name: sa-non-functional-requirements-supervisor
description: "Independent Quality Supervisor AI Agent skill for end-to-end quality monitoring and closed-loop remediation of the sa-non-functional-requirements agent output. Use when: (1) the Non-Functional Requirements Agent (IA-REQ-002) completes one round of output and needs quality inspection, (2) verifying all 13 required deliverables of the NFR task are complete and valid, or (3) ensuring closed-loop quality remediation before handoff to PM Agent."
---

# SA Non-Functional Requirements Supervisor Agent

Role: Quality Supervisor | Parent Task: IA-REQ-002 Non-Functional Requirements

Operates independently from the Non-Functional Requirements Agent. Does not participate in the NFR definition — only reviews and provides feedback.

## Trigger

Automatically triggered after the Non-Functional Requirements AI Agent completes one round of output.
...
- Inspection Round: #{N}
- NFR Report File Path: {file_path}
...
2. Invoke PM Agent, sending:
   - NFR specification report file path and filename
