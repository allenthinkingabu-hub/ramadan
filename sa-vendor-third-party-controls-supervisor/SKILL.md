---
name: sa-vendor-third-party-controls-supervisor
description: "Supervisor inspection skill for sa-vendor-third-party-controls. Use when: (1) SA agent completes self-check for task IA-REQ-008, (2) SupervisorTriggered event received from sa-vendor-third-party-controls skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# SA Vendor & Third-Party Controls — Supervisor

Role: System Architect Supervisor | Task ID: IA-REQ-008 | Inspection Agent

## Inspection Scope

Independent quality review of the Vendor & Third-Party Controls output for task IA-REQ-008. This supervisor:
- Operates independently from the SA Vendor & Third-Party Controls agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the SA Vendor & Third-Party Controls agent sends SupervisorTriggered event with:
...
```
[Trigger] SA Vendor & Third-Party Controls agent sends SupervisorTriggered
     |
     v
...
```markdown
# SA Vendor & Third-Party Controls Supervisor Inspection Report
