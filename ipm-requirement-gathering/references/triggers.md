# Trigger Configuration — IPM-INC-001 Requirement Gathering

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches IPM-INC-001 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests requirement gathering for the project |
...
|----|--------|--------|-------------|
| S-001 | ipm-requirement-gathering-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Requirement Gathering is finalized (after supervisor approval) |
