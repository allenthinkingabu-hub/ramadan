---
name: pm-risk-issue-management-supervisor
description: "Supervisor inspection skill for pm-risk-issue-management. Use when: (1) PM agent completes self-check for task PM-DEV-002, (2) SupervisorTriggered event received from pm-risk-issue-management skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# PM Risk & Issue Management — Supervisor

Role: IT Project Manager Supervisor | Task ID: PM-DEV-002 | Inspection Agent

## Inspection Scope

Independent quality review of the Risk & Issue Management output for task PM-DEV-002. This supervisor:
- Operates independently from the PM Risk & Issue Management agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the PM Risk & Issue Management agent sends SupervisorTriggered event with:
...
```
[Trigger] PM Risk & Issue Management agent sends SupervisorTriggered
     |
     v
...
```markdown
# PM Risk & Issue Management Supervisor Inspection Report
