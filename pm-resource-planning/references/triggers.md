# Trigger Configuration — PM-INC-003 Resource Planning

## Primary Triggers (any one activates)

| ID | Type | Description |
|----|------|-------------|
| T-001 | TaskTriggered event | PM dispatches PM-INC-003 via event bus |
| T-002 | Upstream completion | Upstream prerequisite tasks completed |
| T-003 | User invocation | User explicitly requests resource planning for the project |
...
|----|--------|--------|-------------|
| S-001 | pm-resource-planning-supervisor | SupervisorTriggered | Triggers supervisor review of output |
| S-002 | PM Agent | TaskCompleted | Notifies PM that Resource Planning is finalized (after supervisor approval) |
