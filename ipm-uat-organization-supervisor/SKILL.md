---
name: ipm-uat-organization-supervisor
description: "Supervisor inspection skill for ipm-uat-organization. Use when: (1) IPM agent completes self-check for task IPM-QA-001, (2) SupervisorTriggered event received from ipm-uat-organization skill, (3) quality gate verification needed before TaskCompleted is sent to PM."
---

# IPM UAT Organization — Supervisor

Role: IT Product Manager Supervisor | Task ID: IPM-QA-001 | Inspection Agent

## Inspection Scope

Independent quality review of the UAT Organization output for task IPM-QA-001. This supervisor:
- Operates independently from the IPM UAT Organization agent
- Reviews only — never modifies deliverables directly
- Must achieve 100% pass rate before TaskCompleted can be sent to PM

## Trigger

Activated when the IPM UAT Organization agent sends SupervisorTriggered event with:
...
```
[Trigger] IPM UAT Organization agent sends SupervisorTriggered
     |
     v
...
```markdown
# IPM UAT Organization Supervisor Inspection Report
